...
basedir = '../../../MaterialHash_withIntensity_v2/GTAtoKITTI/kitti/training/'
...
savedir = "../../../dataset3_generated/all/"
...
def main ():
  ...
  for frame in ["000020", "000058", "000042", "000040", "000039", "000031", "000028", "000027", "000015", "000008", "000005"]:
    ...
    # corners3d = corners3d.reshape([int(corners3d.shape[0]/2),4])
    fig = plt.figure()
    ax = fig.add_subplot(projection='3d')
    # plt.title(frame+"_"+str(level))

    # print(levels)
    # print(level)
    # print(corners3d[0,0])

    # pointCloudToBirdsEyeView(ax, velo)
    ...
    plt.savefig(savedir+"/"+frame+"_"+str(level)+"_withBB_BEV_3.png", bbox_inches='tight')
    ...
    # ax.set_xlim(xmin=min(corners3D_X[0])-1,xmax=max(corners3D_X[0])+1)
    # ax.set_ylim(ymin=min(corners3D_X[1])-1,ymax=max(corners3D_X[1])+1)
    # ax.set_zlim(zmin=min(corners3D_X[2])-1,zmax=max(corners3D_X[2])+1)
    
    # # plt.show()
    # plt.savefig(savedir+"/"+frame+"_"+str(level)+"_withBB_object_3.png", bbox_inches='tight')
    ...
    # with open(savedir+"/"+frame+"_"+str(level)+"_github.ply", "w") as f:
    #     f.write(to_write)
